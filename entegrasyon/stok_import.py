...
class ImportSonuc:
    ...
    otomatik_olusturulan: int = 0
    zaten_mevcut: int = 0
    mukerrer_atlanan: int = 0
    gecersiz_ean: int = 0
...
def _barkod_aday(kod: str) -> str | None:
    """Stok kodu 13 haneli rakamsa barkod adayı (metin)."""
    k = normalize_stock_code_as_text(kod)
    if is_13_digit_barcode(k):
        return k
    return None
...
def _mevcut_barkod_listesi(stok: StokKarti | None) -> list[dict]:
    if stok is None:
        return []
    liste = []
    ...
    if not liste and getattr(stok, "barkod", None):
        liste.append(
            {
...
def kaydet_veya_guncelle(
    veriler: dict[str, Any],
    *,
    barkod_istatistik: dict[str, int] | None = None,
) -> str:
    ...
    exclude_id = int(mevcut.id) if mevcut else None
    kaynak_barkodlar = _mevcut_barkod_listesi(mevcut)
    if barkod and normalize_stock_code_as_text(barkod) not in {
        normalize_stock_code_as_text(b.get("barkod")) for b in kaynak_barkodlar
    }:
        # API adayı listede yoksa (manuel olmayan boş kart) senkron kaynağına ekleme; sync halletsin
        pass

    rapor = sync_primary_barcode_from_stock_code(
        kaynak_barkodlar,
        kod,
        birim=(veriler.get("birim") or (mevcut.birim if mevcut else None) or "Adet"),
        exclude_stock_id=exclude_id,
        ask_add=False,
        check_duplicate=True,
    )
    if barkod_istatistik is not None:
        apply_bulk_sync_stats(barkod_istatistik, rapor)

    barkodlar = rapor.get("barkodlar") or []
    # Toplu aktarımda mükerrer barkod satır kaydını düşürmesin
    ...
    if mevcut:
        ...
        try:
            StokService.stok_kaydi(kayit, birlesik, barkodlar=barkodlar)
        except ValueError as exc:
            if "barkod" in str(exc).casefold():
                kayit["barkod"] = mevcut.barkod
                StokService.stok_kaydi(kayit, birlesik, barkodlar=barkodlar or None)
            else:
                raise
    ...
    if barkod and _barkod_baska_kullaniyor_mu(barkod):
        veriler["barkod"] = None
    try:
        kaydedilen = StokService.stok_kaydi(veriler, fiyatlar, barkodlar=barkodlar or None)
    except ValueError as exc:
        if "barkod" in str(exc).casefold():
            veriler["barkod"] = None
            kaydedilen = StokService.stok_kaydi(veriler, fiyatlar, barkodlar=None)
        else:
            raise
...
def aktar_satirlari(satirlar: Iterable[dict[str, Any]]) -> ImportSonuc:
    sonuc = ImportSonuc()
    istatistik = bulk_sync_report()
    for i, satir in enumerate(satirlar, start=1):
        try:
            veriler = map_evobulut_api_satir(satir)
            durum = kaydet_veya_guncelle(veriler, barkod_istatistik=istatistik)
            if durum == "eklendi":
                sonuc.eklenen += 1
            ...
            sonuc.atlanan += 1
    sonuc.otomatik_olusturulan = istatistik["otomatik_olusturulan"]
    sonuc.zaten_mevcut = istatistik["zaten_mevcut"]
    sonuc.mukerrer_atlanan = istatistik["mukerrer_atlanan"]
    sonuc.gecersiz_ean = istatistik["gecersiz_ean"]
    return sonuc
